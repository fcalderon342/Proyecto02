...
public class AccionesUniversales extends ComponentesGenerales {
	...
	public boolean iniciarSesion(String suite, String pruebaCarpeta, String fecha) {
		...
		boolean valor = false;
		// String usuario = "fcalderon342";
		// String contrasena = "saprissa01";
		String usuario = "jcespedes467";
		String contrasena = "Admin012";
		...
		try {
			...
			if (device.equalsIgnoreCase("05b6d672") == true || device.equalsIgnoreCase("09d8788b0b97fa8c") == true
				...
				campoUserLoggin.sendKeys(usuario);
				TimeUnit.SECONDS.sleep(1);
				((AndroidDriver) driver).pressKeyCode(AndroidKeyCode.BACK);

				takeScreenShot(suite, device, pruebaCarpeta, fecha);
				campoPasswordLoggin.click();
				campoPasswordLoggin.sendKeys(contrasena);
				TimeUnit.SECONDS.sleep(1);
				((AndroidDriver) driver).pressKeyCode(AndroidKeyCode.BACK);
			...
			logger.error("NE17", "Error Tecnico Login", e.toString(), suite, pruebaCarpeta, fecha);
	...
	@SuppressWarnings("rawtypes")
	public boolean preguntasFrecuentes(String suite, String pruebaCarpeta, String fecha) {

		boolean valor = false;
		String contrasena = "oracle01";
		...
		try {
			...
			if (device.equalsIgnoreCase("05b6d672") == true || device.equalsIgnoreCase("09d8788b0b97fa8c") == true
				...
				(new WebDriverWait(driver, 3)).until(ExpectedConditions.elementToBeClickable(respuestaPreguntas));
				respuestaPreguntas.click();
				respuestaPreguntas.sendKeys(contrasena);
				TimeUnit.SECONDS.sleep(1);
				((AndroidDriver) driver).pressKeyCode(AndroidKeyCode.BACK);

				takeScreenShot(suite, device, pruebaCarpeta, fecha);
				respuestaEnviar.click();
				TimeUnit.SECONDS.sleep(1);
			...
			logger.error("NE18", "Error Tecnico Login", e.toString(), suite, pruebaCarpeta, fecha);

			valor = false;
		} catch (NoSuchElementException f) {
			logger.error("NE19", "Error Tecnico Preg Frecuentes", f.toString(), suite, pruebaCarpeta, fecha);
		}
		return valor;
	}

	public boolean cercadeMiApp(String suite, String pruebaCarpeta, String fecha) {
		...
		try {
			...
			if (device.equalsIgnoreCase("05b6d672") == true || device.equalsIgnoreCase("09d8788b0b97fa8c") == true
				...
				cercaMi.click();
				aceptarPermisos(suite, pruebaCarpeta, fecha);
				cercaMi.click();
			...
			logger.error("NE20", "Error Tecnico Login", e.toString(), suite, pruebaCarpeta, fecha);

			valor = false;
		} catch (NoSuchElementException f) {
			logger.error("NE21", "Error Tecnico Preg Frecuentes", f.toString(), suite, pruebaCarpeta, fecha);
		}
		return valor;
	}
	...
	public boolean promocionesMis(String suite, String pruebaCarpeta, String fecha) {
		...
		try {
			...
			logger.error("NE22", "Error Tecnico Login", e.toString(), suite, pruebaCarpeta, fecha);

			valor = false;
		} catch (NoSuchElementException f) {
			logger.error("NE2", "Error Tecnico Preg Frecuentes", f.toString(), suite, pruebaCarpeta, fecha);
		}
		return valor;
	}
